...
//                AccountGroupView(
//                    type: type,
//                    accounts: accountsOfType(type),
//                    expandedType: $expandedAccountType
//                )
//            }
//        }
//    }
//
//    private func accountsOfType(_ type: Account.AccountType) -> [Account] {
//        accounts.filter { $0.type == type.rawValue }
//    }
//    
...
struct AccountsView: View {
    ...
    private func accountGroupSection(for type: Account.AccountType) -> some View {
        ...
        let total = calculateGroupBalance(for: type.rawValue)
    ...
    private func accountRow(account: Account) -> some View {
        let currentBalance = calculateAccountBalance(account)
    ...
    /// Calculate balance for a single account (initial + transactions)
    private func calculateAccountBalance(_ account: Account) -> Double {
        ...
        let transactionTotal = accountTransactions.reduce(0.0) { sum, transaction in
    ...
    /// Calculate total balance for an account type (e.g., all "銀行" accounts)
    private func calculateGroupBalance(for type: String) -> Double {
        let accountsOfType = accounts.filter { $0.type == type }
        return accountsOfType.reduce(0) { $0 + calculateAccountBalance($1) }
    }
    
    // MARK: - Computed Properties
    
    private var totalBalance: Double {
        accounts.reduce(0) { $0 + calculateAccountBalance($1) } / 1000
    }
    
    private var totalAssets: Double {
        accounts.filter { calculateAccountBalance($0) > 0 }
            .reduce(0) { $0 + calculateAccountBalance($1) } / 1000
    }
    
    private var totalLiabilities: Double {
        abs(accounts.filter { calculateAccountBalance($0) < 0 }
            .reduce(0) { $0 + calculateAccountBalance($1) }) / 1000
    }
